...
# hardcoded here (not loaded from JSON/a data file) -- matching this project's other
# apworlds (Northgard, WEBFISHING). Ported faithfully from the original Manual-based
# WorldofWarcraft-Leveling project's data/regions.json -- the exact boolean logic each
...
# The two always-reachable ("starting") goal regions, each holding one victory-flagged
# location (see Locations.py). Both exist regardless of which `goal` option is chosen --
# only the completion condition (see Rules.py) depends on that choice.
